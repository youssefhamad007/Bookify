...
export const EventModal = ({ event, onClose, cardPosition }) => {
  ...
  useLayoutEffect(() => {
    ...
    setModalPosition({
      initialX: cardCenterX - viewportCenterX,
      initialY: cardCenterY - viewportCenterY,
      scale: 0.5,
    });
  }, [cardPosition]);
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed left-[-150px] md:left-0 top-0 z-50 w-full h-full flex items-center justify-center p-4 sm:p-8"
    >
      <div
        onClick={onClose}
      ...
      <motion.div
        ...
        className="relative bg-white dark:bg-neutral-900 rounded-xl shadow-xl w-full max-w-lg md:max-w-xl h-[80vh] overflow-y-auto z-10 p-6"
      >
        <button
          onClick={onClose}
